...
func (h *UserHandler) login(c *gin.Context) {
	...
	user, err := h.userSv.Login(context.Background(), req.Tele, req.Password)
	if err != nil {
	...
	c.JSON(http.StatusOK, gin.H{"message": "login success", "name": user.Name})
}
...
func (h *UserHandler) ReplyFriendAdd(c *gin.Context) {
	...
	err := h.userSv.ReplyFriendAdd(context.Background(), userID.(string), req.TargetID, req.Reply)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *UserHandler) ReplyGroupAdd(c *gin.Context) {
	...
	err := h.groupSv.ReplyGroupAdd(context.Background(), userID.(string), req.GroupID, req.Reply)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
